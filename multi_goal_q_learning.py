...
Naive multi-goal finite-history tabular Q-learning.

This is the baseline against which the multi-goal GRU should be compared.

Observable tabular state:
    (goal_id, goal_progress, last L action/outcome pairs)

So the table learns a different Q landscape for each goal, but it shares the
same environment runner and task distribution as the GRU agents.
...
class MultiGoalTabularQAgent:
    name = "multi-goal-tabular-q"
...
def main():
    parser = common_arg_parser("Multi-goal tabular Q-learning")
